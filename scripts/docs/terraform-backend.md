...
| `--download`  | Downloads all available Terraform state files to the `.local/` folder   |
...
Fetches `.tfstate` files for each workspace and stores them in `.local/<workspace>.tfstate`:
```bash
...
- **Clean Exit**: Deletes `.local/` if no files are found during download
